...
const tableName = "tree_planting";

const treePlantingModel = {
  getAll: async (imamProfileId = null) => {
    try {
      ...
      res.rows = res.rows.map((row) => {
        if (row.planting_image && row.planting_image_filename) {
          row.planting_image = "exists";
        } else if (row.planting_image) {
          row.planting_image = row.planting_image.toString("base64");
        }
        return row;
      });
      return res.rows;
...
module.exports = treePlantingModel;
