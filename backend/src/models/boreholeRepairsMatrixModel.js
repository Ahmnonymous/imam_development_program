...
const tableName = "borehole_repairs_matrix";

const boreholeRepairsMatrixModel = {
  getAll: async (boreholeId = null) => {
    try {
      ...
      res.rows = res.rows.map((row) => {
        if (row.task && row.task_filename) {
          row.task = "exists";
        } else if (row.task) {
          row.task = row.task.toString("base64");
        }
        ...
        if (row.parts_image && row.parts_image_filename) {
          row.parts_image = "exists";
        } else if (row.parts_image) {
          row.parts_image = row.parts_image.toString("base64");
        }
        return row;
      });
      return res.rows;
...
module.exports = boreholeRepairsMatrixModel;
