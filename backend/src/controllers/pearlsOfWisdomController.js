...
const pearlsOfWisdomController = {
  getAll: async (req, res) => { 
    try {
      ...
      const data = await pearlsOfWisdomModel.getAll(centerId, isSuperAdmin, imamProfileId); 
      res.json(data); 
    } catch(err){ 
  ...
  getById: async (req, res) => { 
    try {
      const centerId = req.center_id || req.user?.center_id;
      const isSuperAdmin = req.isAppAdmin || false;
      const data = await pearlsOfWisdomModel.getById(req.params.id, centerId, isSuperAdmin); 
  ...
  update: async (req, res) => { 
    try { 
      ...
      const centerId = req.center_id || req.user?.center_id;
      const isSuperAdmin = req.isAppAdmin || false;
      const data = await pearlsOfWisdomModel.update(req.params.id, fields, centerId, isSuperAdmin); 
      if (!data) {
  ...
  delete: async (req, res) => { 
    try {
      const centerId = req.center_id || req.user?.center_id;
      const isSuperAdmin = req.isAppAdmin || false;
      const deleted = await pearlsOfWisdomModel.delete(req.params.id, centerId, isSuperAdmin); 
      if (!deleted) {
        return res.status(404).json({error: 'Not found'}); 
      }
