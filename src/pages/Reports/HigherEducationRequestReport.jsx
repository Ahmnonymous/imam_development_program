...
import { GET_HIGHER_EDUCATION_REQUEST_REPORT } from '../../helpers/url_helper';

const HigherEducationRequestReport = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const response = await axiosApi.get(GET_HIGHER_EDUCATION_REQUEST_REPORT, {
            ...
            console.error('Error fetching higher education request:', err);
            setError(err.response?.data?.message || 'Error fetching higher education request');
        } finally {
            setLoading(false);
        }
    };
    ...
    const processedData = useMemo(() => {
        ...
        if (searchTerm) {
            ...
            result = result.filter(item =>
                ...
                item.institute_name?.toLowerCase().includes(searchLower)
            );
        }
        if (sortConfig.key) {
            result.sort((a, b) => {
    ...
    const exportToCSV = () => {
        const headers = ['Imam Name', 'Imam Surname', 'Course Type', 'Course Name', 'Institute Name', 'Start Date', 'End Date', 'Cost (Local)', 'Cost (ZAR)', 'Status', 'Created At'];
        const csvData = processedData.map(item => [
            item.imam_name || '', item.imam_surname || '', item.course_type || '',
            item.course_name || '', item.institute_name || '',
            formatDate(item.start_date), formatDate(item.end_date),
            item.cost_local_currency || '', item.cost_south_african_rand || '',
        ...
        link.download = `higher_education_request_report_${new Date().toISOString().split('T')[0]}.csv`;
        link.click();
    };
            ...
            <div className="page-content">
                <div className="container-fluid">
                    <div className="d-flex justify-content-center align-items-center" style={{ minHeight: '400px' }}>
                        ...
                        <span>Loading higher education request report...</span>
                    </div>
                </div>
            </div>
        );
    ...
    document.title = "Higher Education Request Report | IDP";

    return (
        <div className="page-content">
            <Container fluid>
                <Breadcrumbs title="Reports" breadcrumbItem="Higher Education Request Report" />
                <Row>
                    <Col lg={12}>
                        <Card>
                            <CardBody>
                                ...
                                {!loading && !error && data.length > 0 && (
                                    <Row className="mb-2">
                                        ...
                                        <Col sm={4}>
                                            <Input type="text" className="form-control search-box me-2 mb-2 d-inline-block" placeholder="Search Higher Education..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                                        </Col>
                                    </Row>
                                )}
                                ...
                                {!loading && !error && data.length === 0 && (
                                    <div className="alert alert-info" role="alert">
                                        ...
                                        No higher education request data found.
                                    </div>
                                )}
                                                            ...
                                                            <tbody>
                                                                {groupItems.map((item) => (
                                                                    <tr key={item.id}>
                                                                        ...
                                                                        <td>{item.cost_south_african_rand || '-'}</td>
...
export default HigherEducationRequestReport;
