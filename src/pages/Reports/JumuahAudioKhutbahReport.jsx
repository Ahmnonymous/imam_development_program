...
import { GET_JUMUAH_AUDIO_KHUTBAH_REPORT } from '../../helpers/url_helper';

const JumuahAudioKhutbahReport = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const response = await axiosApi.get(GET_JUMUAH_AUDIO_KHUTBAH_REPORT, {
            ...
            console.error('Error fetching jumuah audio khutbah:', err);
            setError(err.response?.data?.message || 'Error fetching jumuah audio khutbah');
        } finally {
            setLoading(false);
        }
    };
    ...
    const processedData = useMemo(() => {
        ...
        if (searchTerm) {
            ...
            result = result.filter(item =>
                ...
                item.language_name?.toLowerCase().includes(searchLower)
            );
        }
        if (sortConfig.key) {
            result.sort((a, b) => {
    ...
    const exportToCSV = () => {
        const headers = ['Imam Name', 'Imam Surname', 'Khutbah Topic', 'Date of Khutbah', 'People Present', 'Language', 'Status', 'Created At'];
        const csvData = processedData.map(item => [
            item.imam_name || '', item.imam_surname || '', item.khutbah_topic || '',
            formatDate(item.date_of_khutbah), item.people_present || '',
        ...
        link.download = `jumuah_audio_khutbah_report_${new Date().toISOString().split('T')[0]}.csv`;
        link.click();
    };
            ...
            <div className="page-content">
                <div className="container-fluid">
                    <div className="d-flex justify-content-center align-items-center" style={{ minHeight: '400px' }}>
                        ...
                        <span>Loading jumuah audio khutbah report...</span>
                    </div>
                </div>
            </div>
        );
    ...
    document.title = "Jumuah Audio Khutbah Report | IDP";

    return (
        <div className="page-content">
            <Container fluid>
                <Breadcrumbs title="Reports" breadcrumbItem="Jumuah Audio Khutbah Report" />
                <Row>
                    <Col lg={12}>
                        <Card>
                            <CardBody>
                                ...
                                {!loading && !error && data.length > 0 && (
                                    <Row className="mb-2">
                                        ...
                                        <Col sm={4}>
                                            <Input type="text" className="form-control search-box me-2 mb-2 d-inline-block" placeholder="Search Jumuah Audio Khutbah..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                                        </Col>
                                    </Row>
                                )}
                                ...
                                {!loading && !error && data.length === 0 && (
                                    <div className="alert alert-info" role="alert">
                                        ...
                                        No jumuah audio khutbah data found.
                                    </div>
                                )}
                                                            ...
                                                            <tbody>
                                                                {groupItems.map((item) => (
                                                                    <tr key={item.id}>
                                                                        ...
                                                                        <td>{formatDate(item.date_of_khutbah)}</td>
...
export default JumuahAudioKhutbahReport;
