...
import { GET_TREE_REQUESTS_REPORT } from '../../helpers/url_helper';

const TreeRequestsReport = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const response = await axiosApi.get(GET_TREE_REQUESTS_REPORT, {
            ...
            setError(err.response?.data?.message || 'Error fetching tree requests');
        } finally {
            setLoading(false);
        }
    };
    ...
    const processedData = useMemo(() => {
        ...
        if (searchTerm) {
            ...
            result = result.filter(item =>
                ...
                item.planting_location?.toLowerCase().includes(searchLower) ||
                item.tree_type?.toLowerCase().includes(searchLower)
            );
        }
        if (sortConfig.key) {
            result.sort((a, b) => {
    ...
    const exportToCSV = () => {
        const headers = ['Imam Name', 'Imam Surname', 'Number of Trees', 'Tree Type', 'Planting Location', 'Planting Date', 'Status', 'Created At'];
        const csvData = processedData.map(item => [
            item.imam_name || '', item.imam_surname || '', item.number_of_trees || '',
            item.tree_type || '', item.planting_location || '', formatDate(item.planting_date),
        ...
        link.download = `tree_requests_report_${new Date().toISOString().split('T')[0]}.csv`;
        link.click();
    };
            ...
            <div className="page-content">
                <div className="container-fluid">
                    <div className="d-flex justify-content-center align-items-center" style={{ minHeight: '400px' }}>
                        ...
                        <span>Loading tree requests report...</span>
                    </div>
                </div>
            </div>
        );
    ...
    document.title = "Tree Requests Report | IDP";

    return (
        <div className="page-content">
            <Container fluid>
                <Breadcrumbs title="Reports" breadcrumbItem="Tree Requests Report" />
                <Row>
                    <Col lg={12}>
                        <Card>
                            <CardBody>
                                ...
                                {!loading && !error && data.length > 0 && (
                                    <Row className="mb-2">
                                        ...
                                        <Col sm={4}>
                                            <Input type="text" className="form-control search-box me-2 mb-2 d-inline-block" placeholder="Search Tree Requests..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                                        </Col>
                                    </Row>
                                )}
                                                            ...
                                                            <tbody>
                                                                {groupItems.map((item) => (
                                                                    <tr key={item.id}>
                                                                        ...
                                                                        <td><small>{item.planting_location || '-'}</small></td>
...
export default TreeRequestsReport;
