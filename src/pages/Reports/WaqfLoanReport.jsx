...
import { GET_WAQF_LOAN_REPORT } from '../../helpers/url_helper';

const WaqfLoanReport = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const response = await axiosApi.get(GET_WAQF_LOAN_REPORT, {
            ...
            console.error('Error fetching waqf loan:', err);
            setError(err.response?.data?.message || 'Error fetching waqf loan');
        } finally {
            setLoading(false);
        }
    };
    ...
    const processedData = useMemo(() => {
        ...
        if (searchTerm) {
            ...
            result = result.filter(item =>
                ...
                item.loan_reason?.toLowerCase().includes(searchLower)
            );
        }
        if (sortConfig.key) {
            result.sort((a, b) => {
    ...
    const exportToCSV = () => {
        const headers = ['Imam Name', 'Imam Surname', 'Loan Type', 'Amount Required', 'Monthly Income', 'Monthly Expenses', 'Repayment Structure', 'First Guarantor', 'Second Guarantor', 'Status', 'Created At'];
        const csvData = processedData.map(item => [
            item.imam_name || '', item.imam_surname || '', item.loan_type || '',
            item.amount_required || '', item.monthly_income || '', item.monthly_expenses || '',
            item.repayment_structure || '', item.first_guarantor_name || '', item.second_guarantor_name || '',
        ...
        link.download = `waqf_loan_report_${new Date().toISOString().split('T')[0]}.csv`;
        link.click();
    };
            ...
            <div className="page-content">
                <div className="container-fluid">
                    <div className="d-flex justify-content-center align-items-center" style={{ minHeight: '400px' }}>
                        ...
                        <span>Loading waqf loan report...</span>
                    </div>
                </div>
            </div>
        );
    ...
    document.title = "WAQF Loan Report | IDP";

    return (
        <div className="page-content">
            <Container fluid>
                <Breadcrumbs title="Reports" breadcrumbItem="WAQF Loan Report" />
                <Row>
                    <Col lg={12}>
                        <Card>
                            <CardBody>
                                ...
                                {!loading && !error && data.length > 0 && (
                                    <Row className="mb-2">
                                        ...
                                        <Col sm={4}>
                                            <Input type="text" className="form-control search-box me-2 mb-2 d-inline-block" placeholder="Search WAQF Loan..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                                        </Col>
                                    </Row>
                                )}
                                                            ...
                                                            <tbody>
                                                                {groupItems.map((item) => (
                                                                    <tr key={item.id}>
                                                                        ...
                                                                        <td>{item.repayment_structure || '-'}</td>
