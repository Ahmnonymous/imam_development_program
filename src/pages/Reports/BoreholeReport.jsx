...
import { GET_BOREHOLE_REPORT } from '../../helpers/url_helper';

const BoreholeReport = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const response = await axiosApi.get(GET_BOREHOLE_REPORT, {
            ...
            console.error('Error fetching borehole:', err);
            setError(err.response?.data?.message || 'Error fetching borehole');
        } finally {
            setLoading(false);
        }
    };
    ...
    const processedData = useMemo(() => {
        ...
        if (searchTerm) {
            ...
            result = result.filter(item =>
                ...
                item.borehole_location_name?.toLowerCase().includes(searchLower)
            );
        }
        if (sortConfig.key) {
            result.sort((a, b) => {
    ...
    const exportToCSV = () => {
        const headers = ['Imam Name', 'Imam Surname', 'Borehole Location', 'Has Electricity', 'People Benefiting', 'Status', 'Created At'];
        const csvData = processedData.map(item => [
            item.imam_name || '', item.imam_surname || '', item.borehole_location_name || '',
            item.has_electricity_name || '', item.people_benefiting || '', item.status_name || '',
        ...
        link.download = `borehole_report_${new Date().toISOString().split('T')[0]}.csv`;
        link.click();
    };
            ...
            <div className="page-content">
                <div className="container-fluid">
                    <div className="d-flex justify-content-center align-items-center" style={{ minHeight: '400px' }}>
                        ...
                        <span>Loading borehole report...</span>
                    </div>
                </div>
            </div>
        );
    ...
    document.title = "Borehole Report | IDP";

    return (
        <div className="page-content">
            <Container fluid>
                <Breadcrumbs title="Reports" breadcrumbItem="Borehole Report" />
                <Row>
                    <Col lg={12}>
                        <Card>
                            <CardBody>
                                            ...
                                            <Col md={6}>
                                                <FormGroup>
                                                    ...
                                                    <Input type="select" value={groupBy} onChange={(e) => setGroupBy(e.target.value)}>
                                                        ...
                                                        <option value="borehole_location_name">Borehole Location</option>
                                ...
                                {!loading && !error && data.length > 0 && (
                                    <Row className="mb-2">
                                        ...
                                        <Col sm={4}>
                                            <Input type="text" className="form-control search-box me-2 mb-2 d-inline-block" placeholder="Search Borehole..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                                        </Col>
                                    </Row>
                                )}
                                                        ...
                                                        <Table hover className="table-bordered table-nowrap table-sm">
                                                            <thead className="table-light" style={{ position: 'sticky', top: 0, zIndex: 1 }}>
                                                                <tr>
                                                                    ...
                                                                    <th>Imam Surname</th>
                                                                    <th>Borehole Location</th>
                                                                    <th>Has Electricity</th>
                                                                    <th>People Benefiting</th>
                                                                    <th>Status</th>
                                                                    <th>Created At</th>
                                                                </tr>
                                                            </thead>
                                                            <tbody>
                                                                {groupItems.map((item) => (
                                                                    <tr key={item.id}>
                                                                        ...
                                                                        <td>{item.borehole_location_name || '-'}</td>
                                                                        <td>{item.has_electricity_name || '-'}</td>
                                                                        <td>{item.people_benefiting || '-'}</td>
