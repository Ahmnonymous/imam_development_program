...
import { GET_NEW_MUSLIM_BONUS_REPORT } from '../../helpers/url_helper';

const NewMuslimBonusReport = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const response = await axiosApi.get(GET_NEW_MUSLIM_BONUS_REPORT, {
            ...
            console.error('Error fetching new muslim bonus:', err);
            setError(err.response?.data?.message || 'Error fetching new muslim bonus');
        } finally {
            setLoading(false);
        }
    };
    ...
    const processedData = useMemo(() => {
        ...
        if (searchTerm) {
            ...
            result = result.filter(item =>
                ...
                item.revert_name?.toLowerCase().includes(searchLower) ||
                item.revert_email?.toLowerCase().includes(searchLower)
            );
        }
        if (sortConfig.key) {
            result.sort((a, b) => {
    ...
    const exportToCSV = () => {
        const headers = ['Imam Name', 'Imam Surname', 'Revert Name', 'Revert Gender', 'Revert DOB', 'Revert Phone', 'Revert Email', 'Revert Reason', 'Status', 'Created At'];
        const csvData = processedData.map(item => [
            item.imam_name || '', item.imam_surname || '', item.revert_name || '',
            item.revert_gender_name || '', formatDate(item.revert_dob),
            item.revert_phone || '', item.revert_email || '', item.revert_reason || '',
        ...
        link.download = `new_muslim_bonus_report_${new Date().toISOString().split('T')[0]}.csv`;
        link.click();
    };
            ...
            <div className="page-content">
                <div className="container-fluid">
                    <div className="d-flex justify-content-center align-items-center" style={{ minHeight: '400px' }}>
                        ...
                        <span>Loading new muslim bonus report...</span>
                    </div>
                </div>
            </div>
        );
    ...
    document.title = "New Muslim Bonus Report | IDP";

    return (
        <div className="page-content">
            <Container fluid>
                <Breadcrumbs title="Reports" breadcrumbItem="New Muslim Bonus Report" />
                <Row>
                    <Col lg={12}>
                        <Card>
                            <CardBody>
                                            ...
                                            <Col md={6}>
                                                <FormGroup>
                                                    ...
                                                    <Input type="select" value={groupBy} onChange={(e) => setGroupBy(e.target.value)}>
                                                        ...
                                                        <option value="revert_gender_name">Revert Gender</option>
                                ...
                                {!loading && !error && data.length > 0 && (
                                    <Row className="mb-2">
                                        ...
                                        <Col sm={4}>
                                            <Input type="text" className="form-control search-box me-2 mb-2 d-inline-block" placeholder="Search New Muslim Bonus..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                                        </Col>
                                    </Row>
                                )}
                                ...
                                {!loading && !error && data.length === 0 && (
                                    <div className="alert alert-info" role="alert">
                                        ...
                                        No new muslim bonus data found.
                                    </div>
                                )}
                                                            ...
                                                            <tbody>
                                                                {groupItems.map((item) => (
                                                                    <tr key={item.id}>
                                                                        ...
                                                                        <td>{item.revert_gender_name || '-'}</td>
                                                                        <td>{formatDate(item.revert_dob)}</td>
                                                                        <td>{item.revert_phone || '-'}</td>
                                                                        <td><small>{item.revert_email || '-'}</small></td>
...
export default NewMuslimBonusReport;
