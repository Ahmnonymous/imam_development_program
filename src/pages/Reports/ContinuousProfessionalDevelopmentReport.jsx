...
import { GET_CONTINUOUS_PROFESSIONAL_DEVELOPMENT_REPORT } from '../../helpers/url_helper';

const ContinuousProfessionalDevelopmentReport = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const response = await axiosApi.get(GET_CONTINUOUS_PROFESSIONAL_DEVELOPMENT_REPORT, {
            ...
            console.error('Error fetching continuous professional development:', err);
            setError(err.response?.data?.message || 'Error fetching continuous professional development');
        } finally {
            setLoading(false);
        }
    };
    ...
    const exportToCSV = () => {
        const headers = ['Imam Name', 'Imam Surname', 'Institution Name', 'Start Date', 'End Date', 'Fees (Local)', 'Fees (ZAR)', 'Status', 'Created At'];
        const csvData = processedData.map(item => [
            item.imam_name || '', item.imam_surname || '', item.institution_name || '',
            formatDate(item.start_date), formatDate(item.end_date),
            item.fees_local_currency || '', item.fees_south_african_rand || '',
        ...
        link.download = `continuous_professional_development_report_${new Date().toISOString().split('T')[0]}.csv`;
        link.click();
    };
            ...
            <div className="page-content">
                <div className="container-fluid">
                    <div className="d-flex justify-content-center align-items-center" style={{ minHeight: '400px' }}>
                        ...
                        <span>Loading continuous professional development report...</span>
                    </div>
                </div>
            </div>
        );
    ...
    document.title = "Continuous Professional Development Report | IDP";

    return (
        <div className="page-content">
            <Container fluid>
                <Breadcrumbs title="Reports" breadcrumbItem="Continuous Professional Development Report" />
                <Row>
                    <Col lg={12}>
                        <Card>
                            <CardBody>
                                ...
                                {!loading && !error && data.length > 0 && (
                                    <Row className="mb-2">
                                        ...
                                        <Col sm={4}>
                                            <Input type="text" className="form-control search-box me-2 mb-2 d-inline-block" placeholder="Search CPD..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                                        </Col>
                                    </Row>
                                )}
                                ...
                                {!loading && !error && data.length === 0 && (
                                    <div className="alert alert-info" role="alert">
                                        ...
                                        No continuous professional development data found.
                                    </div>
                                )}
                                                            ...
                                                            <tbody>
                                                                {groupItems.map((item) => (
                                                                    <tr key={item.id}>
                                                                        ...
                                                                        <td>{item.fees_local_currency || '-'}</td>
                                                                        <td>{item.fees_south_african_rand || '-'}</td>
...
export default ContinuousProfessionalDevelopmentReport;
