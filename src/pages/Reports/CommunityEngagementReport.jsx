...
import { GET_COMMUNITY_ENGAGEMENT_REPORT } from '../../helpers/url_helper';

const CommunityEngagementReport = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const response = await axiosApi.get(GET_COMMUNITY_ENGAGEMENT_REPORT, {
            ...
            console.error('Error fetching community engagement:', err);
            setError(err.response?.data?.message || 'Error fetching community engagement');
        } finally {
            setLoading(false);
        }
    };
    ...
    const processedData = useMemo(() => {
        ...
        if (searchTerm) {
            ...
            result = result.filter(item =>
                ...
                item.engagement_type_name?.toLowerCase().includes(searchLower)
            );
        }
        if (sortConfig.key) {
            result.sort((a, b) => {
    ...
    const exportToCSV = () => {
        const headers = ['Imam Name', 'Imam Surname', 'Engagement Type', 'People Count', 'Engagement Date', 'Status', 'Created At'];
        const csvData = processedData.map(item => [
            item.imam_name || '', item.imam_surname || '', item.engagement_type_name || '',
            item.people_count || '', formatDate(item.engagement_date), item.status_name || '',
        ...
        link.download = `community_engagement_report_${new Date().toISOString().split('T')[0]}.csv`;
        link.click();
    };
            ...
            <div className="page-content">
                <div className="container-fluid">
                    <div className="d-flex justify-content-center align-items-center" style={{ minHeight: '400px' }}>
                        ...
                        <span>Loading community engagement report...</span>
                    </div>
                </div>
            </div>
        );
    ...
    document.title = "Community Engagement Report | IDP";

    return (
        <div className="page-content">
            <Container fluid>
                <Breadcrumbs title="Reports" breadcrumbItem="Community Engagement Report" />
                <Row>
                    <Col lg={12}>
                        <Card>
                            <CardBody>
                                            ...
                                            <Col md={6}>
                                                <FormGroup>
                                                    ...
                                                    <Input type="select" value={groupBy} onChange={(e) => setGroupBy(e.target.value)}>
                                                        ...
                                                        <option value="engagement_type_name">Engagement Type</option>
                                ...
                                {!loading && !error && data.length > 0 && (
                                    <Row className="mb-2">
                                        ...
                                        <Col sm={4}>
                                            <Input type="text" className="form-control search-box me-2 mb-2 d-inline-block" placeholder="Search Community Engagement..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                                        </Col>
                                    </Row>
                                )}
                                ...
                                {!loading && !error && data.length === 0 && (
                                    <div className="alert alert-info" role="alert">
                                        ...
                                        No community engagement data found.
                                    </div>
                                )}
                                                            ...
                                                            <tbody>
                                                                {groupItems.map((item) => (
                                                                    <tr key={item.id}>
                                                                        ...
                                                                        <td>{item.engagement_type_name || '-'}</td>
...
export default CommunityEngagementReport;
