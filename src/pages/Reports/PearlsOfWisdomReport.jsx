...
import { GET_PEARLS_OF_WISDOM_REPORT } from '../../helpers/url_helper';

const PearlsOfWisdomReport = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const response = await axiosApi.get(GET_PEARLS_OF_WISDOM_REPORT, {
            ...
            console.error('Error fetching pearls of wisdom:', err);
            setError(err.response?.data?.message || 'Error fetching pearls of wisdom');
        } finally {
            setLoading(false);
        }
    };
    ...
    const processedData = useMemo(() => {
        ...
        if (searchTerm) {
            ...
            result = result.filter(item =>
                ...
                item.resource_title?.toLowerCase().includes(searchLower) ||
                item.author_speaker?.toLowerCase().includes(searchLower) ||
                item.resource_type_name?.toLowerCase().includes(searchLower)
            );
        }
        if (sortConfig.key) {
            result.sort((a, b) => {
    ...
    const exportToCSV = () => {
        const headers = ['Imam Name', 'Imam Surname', 'Resource Type', 'Resource Title', 'Author/Speaker', 'Heading Description', 'Status', 'Created At'];
        const csvData = processedData.map(item => [
            item.imam_name || '', item.imam_surname || '', item.resource_type_name || '',
            item.resource_title || '', item.author_speaker || '', item.heading_description || '',
        ...
        link.download = `pearls_of_wisdom_report_${new Date().toISOString().split('T')[0]}.csv`;
        link.click();
    };
            ...
            <div className="page-content">
                <div className="container-fluid">
                    <div className="d-flex justify-content-center align-items-center" style={{ minHeight: '400px' }}>
                        ...
                        <span>Loading pearls of wisdom report...</span>
                    </div>
                </div>
            </div>
        );
    ...
    document.title = "Pearls of Wisdom Report | IDP";

    return (
        <div className="page-content">
            <Container fluid>
                <Breadcrumbs title="Reports" breadcrumbItem="Pearls of Wisdom Report" />
                <Row>
                    <Col lg={12}>
                        <Card>
                            <CardBody>
                                            ...
                                            <Col md={6}>
                                                <FormGroup>
                                                    ...
                                                    <Input type="select" value={groupBy} onChange={(e) => setGroupBy(e.target.value)}>
                                                        ...
                                                        <option value="resource_type_name">Resource Type</option>
                                ...
                                {!loading && !error && data.length > 0 && (
                                    <Row className="mb-2">
                                        ...
                                        <Col sm={4}>
                                            <Input type="text" className="form-control search-box me-2 mb-2 d-inline-block" placeholder="Search Pearls of Wisdom..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                                        </Col>
                                    </Row>
                                )}
                                ...
                                {!loading && !error && data.length === 0 && (
                                    <div className="alert alert-info" role="alert">
                                        ...
                                        No pearls of wisdom data found.
                                    </div>
                                )}
...
export default PearlsOfWisdomReport;
