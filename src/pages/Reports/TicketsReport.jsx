...
import { GET_TICKETS_REPORT } from '../../helpers/url_helper';
...
const TicketsReport = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const response = await axiosApi.get(GET_TICKETS_REPORT, {
    ...
    const processedData = useMemo(() => {
        ...
        if (searchTerm) {
            ...
            result = result.filter(item =>
                ...
                item.classification_name?.toLowerCase().includes(searchLower) ||
                item.allocated_to_name?.toLowerCase().includes(searchLower) ||
    ...
    const exportToCSV = () => {
        const headers = ['ID', 'Classification', 'Description', 'Status', 'Allocated To', 'Created At', 'Closed At'];
        const csvData = processedData.map(item => [
            item.id || '', item.classification_name || '', item.description || '',
            item.status_name || '', item.allocated_to_name ? `${item.allocated_to_name} ${item.allocated_to_surname}` : '',
            formatDate(item.created_at), formatDate(item.closed_at)
        ]);
            ...
            <div className="page-content">
                <div className="container-fluid">
                    <div className="d-flex justify-content-center align-items-center" style={{ minHeight: '400px' }}>
                        ...
                        <span>Loading tickets report...</span>
                    </div>
                </div>
            </div>
        );
    ...
    document.title = "Tickets Report | IDP";

    return (
        <div className="page-content">
            <Container fluid>
                <Breadcrumbs title="Reports" breadcrumbItem="Tickets Report" />
                <Row>
                    <Col lg={12}>
                        <Card>
                            <CardBody>
                                            ...
                                            <Col md={6}>
                                                <FormGroup>
                                                    ...
                                                    <Input type="select" value={groupBy} onChange={(e) => setGroupBy(e.target.value)}>
                                                        ...
                                                        <option value="allocated_to_name">Allocated To</option>
                                                    </Input>
                                                </FormGroup>
                                            </Col>
                                        </Row>
                                ...
                                {!loading && !error && data.length > 0 && (
                                    <Row className="mb-2">
                                        ...
                                        <Col sm={4}>
                                            <Input type="text" className="form-control search-box me-2 mb-2 d-inline-block" placeholder="Search Tickets..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                                        </Col>
                                    </Row>
                                )}
                                                            ...
                                                            <tbody>
                                                                {groupItems.map((item) => (
                                                                    <tr key={item.id}>
                                                                        ...
                                                                        <td><Badge color={item.status_name === 'Closed' ? 'success' : item.status_name === 'In Progress' ? 'warning' : 'secondary'}>{item.status_name || '-'}</Badge></td>
                                                                        <td>{item.allocated_to_name ? `${item.allocated_to_name} ${item.allocated_to_surname}` : '-'}</td>
