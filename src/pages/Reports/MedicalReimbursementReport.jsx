...
import { GET_MEDICAL_REIMBURSEMENT_REPORT } from '../../helpers/url_helper';

const MedicalReimbursementReport = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const response = await axiosApi.get(GET_MEDICAL_REIMBURSEMENT_REPORT, {
            ...
            console.error('Error fetching medical reimbursement:', err);
            setError(err.response?.data?.message || 'Error fetching medical reimbursement');
        } finally {
            setLoading(false);
        }
    };
    ...
    const processedData = useMemo(() => {
        ...
        if (searchTerm) {
            ...
            result = result.filter(item =>
                ...
                item.visit_type_name?.toLowerCase().includes(searchLower) ||
                item.service_provider_name?.toLowerCase().includes(searchLower)
            );
        }
        if (sortConfig.key) {
            result.sort((a, b) => {
    ...
    const exportToCSV = () => {
        const headers = ['Imam Name', 'Imam Surname', 'Relationship Type', 'Visit Type', 'Visit Date', 'Service Provider', 'Amount', 'Status', 'Created At'];
        const csvData = processedData.map(item => [
            item.imam_name || '', item.imam_surname || '', item.relationship_type_name || '',
            item.visit_type_name || '', formatDate(item.visit_date),
            item.service_provider_name || '', item.amount || '', item.status_name || '',
        ...
        link.download = `medical_reimbursement_report_${new Date().toISOString().split('T')[0]}.csv`;
        link.click();
    };
            ...
            <div className="page-content">
                <div className="container-fluid">
                    <div className="d-flex justify-content-center align-items-center" style={{ minHeight: '400px' }}>
                        ...
                        <span>Loading medical reimbursement report...</span>
                    </div>
                </div>
            </div>
        );
    ...
    document.title = "Medical Reimbursement Report | IDP";

    return (
        <div className="page-content">
            <Container fluid>
                <Breadcrumbs title="Reports" breadcrumbItem="Medical Reimbursement Report" />
                <Row>
                    <Col lg={12}>
                        <Card>
                            <CardBody>
                                            ...
                                            <Col md={6}>
                                                <FormGroup>
                                                    ...
                                                    <Input type="select" value={groupBy} onChange={(e) => setGroupBy(e.target.value)}>
                                                        ...
                                                        <option value="visit_type_name">Visit Type</option>
                                ...
                                {!loading && !error && data.length > 0 && (
                                    <Row className="mb-2">
                                        ...
                                        <Col sm={4}>
                                            <Input type="text" className="form-control search-box me-2 mb-2 d-inline-block" placeholder="Search Medical Reimbursement..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                                        </Col>
                                    </Row>
                                )}
                                ...
                                {!loading && !error && data.length === 0 && (
                                    <div className="alert alert-info" role="alert">
                                        ...
                                        No medical reimbursement data found.
                                    </div>
                                )}
                                                            ...
                                                            <tbody>
                                                                {groupItems.map((item) => (
                                                                    <tr key={item.id}>
                                                                        ...
                                                                        <td>{item.relationship_type_name || '-'}</td>
...
export default MedicalReimbursementReport;
