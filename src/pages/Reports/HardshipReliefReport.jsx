...
import { GET_HARDSHIP_RELIEF_REPORT } from '../../helpers/url_helper';

const HardshipReliefReport = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const response = await axiosApi.get(GET_HARDSHIP_RELIEF_REPORT, {
            ...
            console.error('Error fetching hardship relief:', err);
            setError(err.response?.data?.message || 'Error fetching hardship relief');
        } finally {
            setLoading(false);
        }
    };
    ...
    const processedData = useMemo(() => {
        ...
        if (searchTerm) {
            ...
            result = result.filter(item =>
                ...
                item.name_of_person_community?.toLowerCase().includes(searchLower) ||
                item.assistance_type?.toLowerCase().includes(searchLower)
            );
        }
        if (sortConfig.key) {
            result.sort((a, b) => {
    ...
    const exportToCSV = () => {
        const headers = ['Imam Name', 'Imam Surname', 'Request For', 'Name of Person/Community', 'Assistance Type', 'Amount Required', 'Status', 'Created At'];
        const csvData = processedData.map(item => [
            item.imam_name || '', item.imam_surname || '', item.request_for_name || '',
            item.name_of_person_community || '', item.assistance_type || '',
            item.amount_required_local_currency || '', item.status_name || '',
        ...
        link.download = `hardship_relief_report_${new Date().toISOString().split('T')[0]}.csv`;
        link.click();
    };
            ...
            <div className="page-content">
                <div className="container-fluid">
                    <div className="d-flex justify-content-center align-items-center" style={{ minHeight: '400px' }}>
                        ...
                        <span>Loading hardship relief report...</span>
                    </div>
                </div>
            </div>
        );
    ...
    document.title = "Hardship Relief Report | IDP";

    return (
        <div className="page-content">
            <Container fluid>
                <Breadcrumbs title="Reports" breadcrumbItem="Hardship Relief Report" />
                <Row>
                    <Col lg={12}>
                        <Card>
                            <CardBody>
                                            ...
                                            <Col md={6}>
                                                <FormGroup>
                                                    ...
                                                    <Input type="select" value={groupBy} onChange={(e) => setGroupBy(e.target.value)}>
                                                        ...
                                                        <option value="request_for_name">Request For</option>
                                ...
                                {!loading && !error && data.length > 0 && (
                                    <Row className="mb-2">
                                        ...
                                        <Col sm={4}>
                                            <Input type="text" className="form-control search-box me-2 mb-2 d-inline-block" placeholder="Search Hardship Relief..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                                        </Col>
                                    </Row>
                                )}
                                                        ...
                                                        <Table hover className="table-bordered table-nowrap table-sm">
                                                            <thead className="table-light" style={{ position: 'sticky', top: 0, zIndex: 1 }}>
                                                                <tr>
                                                                    ...
                                                                    <th>Imam Surname</th>
                                                                    <th>Request For</th>
                                                                    <th>Name of Person/Community</th>
                                                            ...
                                                            <tbody>
                                                                {groupItems.map((item) => (
                                                                    <tr key={item.id}>
                                                                        ...
                                                                        <td>{item.request_for_name || '-'}</td>
                                                                        <td>{item.name_of_person_community || '-'}</td>
                                                                        <td>{item.assistance_type || '-'}</td>
                                                                        <td>{item.amount_required_local_currency || '-'}</td>
...
export default HardshipReliefReport;
