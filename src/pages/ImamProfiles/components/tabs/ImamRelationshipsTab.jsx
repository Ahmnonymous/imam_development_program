...
const ImamRelationshipsTab = ({ imamProfileId, relationships, lookupData, onUpdate, showAlert }) => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (editItem) {
        ...
        await axiosApi.put(`${API_BASE_URL}/imamRelationships/${editItem.id}`, payload);
  ...
  const handleDelete = () => {
    ...
    const relationshipName = `${editItem.name || 'Unknown Name'} - ${getLookupName(lookupData.relationshipTypes, editItem.relationship_type)}`;
    ...
    showDeleteConfirmation({
      ...
      await axiosApi.delete(`${API_BASE_URL}/imamRelationships/${editItem.id}`);
  ...
  const handleApprove = async (item) => {
    try {
      await axiosApi.put(`${API_BASE_URL}/imamRelationships/${item.id}`, {
        status_id: 2,
        updated_by: getAuditName()
      });
      showAlert("Relationship approved successfully", "success");
      onUpdate();
    } catch (error) {
      showAlert(error?.response?.data?.error || "Failed to approve relationship", "danger");
    }
  };
  ...
  const handleDecline = async (item) => {
    try {
      ...
      showAlert("Relationship declined successfully", "success");
      onUpdate();
    } catch (error) {
      showAlert(error?.response?.data?.error || "Failed to decline relationship", "danger");
    }
  };
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        enableColumnFilter: false,
        cell: (cell) => {
          const status = getLookupName(lookupData?.status, cell.getValue());
...
export default ImamRelationshipsTab;
