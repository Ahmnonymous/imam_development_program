...
const NewBabyBonusTab = ({ imamProfileId, newBabyBonus, lookupData, onUpdate, showAlert }) => {
  ...
  const handleApprove = async (item) => {
    try {
      await axiosApi.put(`${API_BASE_URL}/newBabyBonus/${item.id}`, {
        status_id: 2,
        updated_by: getAuditName()
      });
      showAlert("New baby bonus approved successfully", "success");
      onUpdate();
    } catch (error) {
      showAlert(error?.response?.data?.error || "Failed to approve new baby bonus", "danger");
    }
  };
  ...
  const handleDecline = async (item) => {
    try {
      ...
      showAlert("New baby bonus declined successfully", "success");
      onUpdate();
    } catch (error) {
      showAlert(error?.response?.data?.error || "Failed to decline new baby bonus", "danger");
    }
  };
