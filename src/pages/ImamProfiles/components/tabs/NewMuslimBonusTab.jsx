...
const NewMuslimBonusTab = ({ imamProfileId, newMuslimBonus, lookupData, onUpdate, showAlert }) => {
  ...
  const handleApprove = async (item) => {
    try {
      await axiosApi.put(`${API_BASE_URL}/newMuslimBonus/${item.id}`, {
        status_id: 2,
        updated_by: getAuditName()
      });
      showAlert("New Muslim bonus approved successfully", "success");
      onUpdate();
    } catch (error) {
      showAlert(error?.response?.data?.error || "Failed to approve new Muslim bonus", "danger");
    }
  };
  ...
  const handleDecline = async (item) => {
    try {
      ...
      showAlert("New Muslim bonus declined successfully", "success");
      onUpdate();
    } catch (error) {
      showAlert(error?.response?.data?.error || "Failed to decline new Muslim bonus", "danger");
    }
  };
