...
const MedicalReimbursementTab = ({ imamProfileId, medicalReimbursement, lookupData, onUpdate, showAlert }) => {
  ...
  const handleApprove = async (item) => {
    try {
      await axiosApi.put(`${API_BASE_URL}/medicalReimbursement/${item.id}`, {
        status_id: 2,
        updated_by: getAuditName()
      });
      showAlert("Medical reimbursement approved successfully", "success");
      onUpdate();
    } catch (error) {
      showAlert(error?.response?.data?.error || "Failed to approve medical reimbursement", "danger");
    }
  };
  ...
  const handleDecline = async (item) => {
    try {
      ...
      showAlert("Medical reimbursement declined successfully", "success");
      onUpdate();
    } catch (error) {
      showAlert(error?.response?.data?.error || "Failed to decline medical reimbursement", "danger");
    }
  };
